...
namespace EasyInnerSDK.Entity
{
    public class Inner
    {
        ...
        public override string ToString()
        {
            ...
            return "Inner: " + this.Numero.ToString() +
                   " | Dígitos: " + this.QtdDigitos.ToString() +
                   //" | Catraca: " + this.Catraca.ToString() +
                   " | " + (this.Catraca ? "Catraca" : "Coletor ") +
                   " | " + (this.Biometrico ? "Com bio" : "Sem bio") +
                   " | Verificação: " + (this.Verificacao == 0 ? "Não" : "Sim") +
                   " | Identificação: " + (this.Identificacao == 0 ? "Não" : "Sim") +
                   " | Leitor: " + (this.TipoLeitor == 0 ? "Barras" : (this.TipoLeitor == 1 ? "Magnético" :
                   (this.TipoLeitor == 2 ? "Abatrack" : this.TipoLeitor == 3 ? "Wiegand" : (this.TipoLeitor == 8? "Barras, Prox, QR Code" :"Smart Card")))) +
                   " | " + (this.DoisLeitores ? "Dois Leitores" : "Um Leitor") +
                   " | Lista Off: " + (this.Lista ? "Sim" : "Não") +
